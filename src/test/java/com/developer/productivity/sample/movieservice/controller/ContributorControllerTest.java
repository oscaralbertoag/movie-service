...
class ContributorControllerTest extends BaseControllerTest {
    ...
    public void getAllContributorsReturns200AndMappedResources() {
        ...
        assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        // AND: multiple contributors are returned
        ArrayNode arrayNode = (ArrayNode) getJsonResponse(responseEntity.getBody());
        assertNotNull(arrayNode);
        ...
        arrayNode.forEach(
                jsonNode -> {
                    ...
                    names.add(jsonNode.get("firstName").textValue() + " " + jsonNode.get("lastName").asText());
                });
        // AND: the expected values are returned
        List<String> expectedNames = Arrays.asList("John Smith", "Jane Smith");
